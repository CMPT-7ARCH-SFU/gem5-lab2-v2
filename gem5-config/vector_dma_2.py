...
def buildvector_dma_2_clstr(options, system, clstr):

	local_low = 0x10020000
	local_high = 0x10020140
	...
	# vector_dma_2 Config
	clstr.vector_dma_2.pio = clstr.local_bus.mem_side_ports
	clstr.vector_dma_2.enable_debug_msgs = False
	
	# MATRIX1 (Variable)
	addr = 0x10020080
	spmRange = AddrRange(addr, addr + 0x20)
	...
	# Connecting MATRIX1 to vector_dma_2
	for i in range(2):
		clstr.vector_dma_2.spm = clstr.matrix1.spm_ports
	...
	# Connecting MATRIX2 to vector_dma_2
	for i in range(2):
		clstr.vector_dma_2.spm = clstr.matrix2.spm_ports
	...
	# Connecting MATRIX3 to vector_dma_2
	for i in range(2):
		clstr.vector_dma_2.spm = clstr.matrix3.spm_ports
	
def makeHWAcc(args, system):

	system.vector_dma_2_clstr = AccCluster()
	buildvector_dma_2_clstr(args, system, system.vector_dma_2_clstr)
