...
def buildvector_stream_clstr(options, system, clstr):
	...
	# top Definition
	acc = "top"
	ir = os.environ["LAB_PATH"]+"/benchmarks/vector_stream/hw/top.ll"
	config = os.envir = os.environ["LAB_PATH"]+"/benchmarks/vector_stream/config.yml"
	clstr.top = CommInterface(devicename=acc, gic=gic, pio_addr=0x100200c0, pio_size=64, int_num=68)
	AccConfig(clstr.top, ir, config)
	
	# s1 Definition
	acc = "s1"
	ir = os.environ["LAB_PATH"]+"/benchmarks/vector_stream/hw/S1.ll"
	config = os.envir = os.environ["LAB_PATH"]+"/benchmarks/vector_stream/config.yml"
	...
	# s2 Definition
	acc = "s2"
	ir = os.environ["LAB_PATH"]+"/benchmarks/vector_stream/hw/S2.ll"
	config = os.envir = os.environ["LAB_PATH"]+"/benchmarks/vector_stream/config.yml"
	...
	# s3 Definition
	acc = "s3"
	ir = os.environ["LAB_PATH"]+"/benchmarks/vector_stream/hw/S3.ll"
	config = os.envir = os.environ["LAB_PATH"]+"/benchmarks/vector_stream/config.yml"
